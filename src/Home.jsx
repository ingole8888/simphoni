...
useEffect(() => {
  ...
  const fetchData = async () => {
    const options = {
      ...
      headers: {
        'X-RapidAPI-Key': '0df6d73ba5msh9b654600548eaa9p1201dajsn76fdbffb0005',
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + itemsPerPage < products.length ? prevIndex + itemsPerPage : prevIndex));
  };
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="p-4">
          <h2 className="text-xl font-bold mb-4">Best Selling items and New Arrivals</h2>
          <div className="p-4">
            <div>
              <button className="mr-2" onClick={handlePrev}><FaArrowLeft /> </button>
                      ...
                      <div className="border p-4 flex flex-col justify-between">
                        ...
                        <div>
                          <div className="h-1/10 overflow-hidden">
                            <p className="text-sm font-bold mb-1 clamp-2">{product.name}</p>
                          </div>
                          <p className="mb-2">{product.sku}</p>
                          <h4 className="text-l font-bold mb-2">${product.pricing.customerPrice.unitPrice.value}/each</h4>
                          <p className="text-sm font-bold mb-2 text-teal-500">Saving : {product.customer_reviews.average_rating_value}%</p>
                          <p className="text-sm font-bold mb-2">Supplier : {product.manufacturer.name}</p>
                          {product.shipping.messages && product.shipping.messages.length > 1 ? (
                            <p className="text-sm font-bold mb-2">{product.shipping.messages[1].text}</p>
                          ) : (
          ...
          <div className="p-4">
            ...
            <div className="flex flex-wrap">
              {
                products
                  .slice(currentIndex, currentIndex + itemsPerPage1)
                  .map((product, index) => (
                    <div
                      key={index}
                      className="w-full sm:w-1/2 md:w-1/3 lg:w-1/5 p-2">
            ...
            <div>
              <Link to="/product-listing" className="mx-2"><button >see more ...</button></Link>
            </div>
          </div>
          <h2 className="text-xl font-bold mb-4">Best Selling Categories and Top Suppliers:</h2>
          <div className="p-4">
            <div>
              <button className="mr-2" onClick={handlePrev}><FaArrowLeft /> </button>
              <button onClick={handleNext}><FaArrowRight /></button>
